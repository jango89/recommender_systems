...
public class ElasticSearchDataSource {
	...
	/**
	 * This constructor helps to set the elasticsearchtemplate and client as the
	 * one passed in arguments
	 */
	...
	private ElasticsearchTemplate createElasticSearchTemplate(Environment env, String dsName) {
		...
		Settings settings = Settings.builder().put("cluster.name", env.getProperty(dsName + ".es.clustername")).build();
		...
		for (String esIP : allIps) {
			((TransportClient) client).addTransportAddress(new InetSocketTransportAddress(
					new InetSocketAddress(esIP, Integer.valueOf(env.getProperty(dsName + ".es.port")))));
		}
	 ...
	 * 
	 * The filters when added with post-filter, the result will get reflected
	 * under the Hits session of aggregation response. The filters when added
	 * with aggregations, the result will get reflected under the Aggregation
	 * session of aggregation response.
	 * 
	 * @param query
	 * @param indexName
	 * @param type
	 ...
	 * @throws Exception
	 */
	public <E extends Serializable> SearchResponse generateElasticSearchResponse(NativeSearchQuery query,
			String indexName, String type) throws Exception {
		...
		if (null != this.client) {
			...
			if (null != query.getAggregations()) {
				for (AbstractAggregationBuilder aggBuilder : query.getAggregations()) {
					searchRequest.addAggregation(aggBuilder);
				}
			}
	 ...
	 * 
	 */
	public static <E extends Serializable> SearchHits generateElasticSearchFunctionalScore(SearchQuery query,
			ElasticsearchTemplate elasticsearchTemplateSentOver) throws Exception {
